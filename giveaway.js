...
module.exports.run = async (bot, message, args) => {
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Извините, вы не можете этого сделать!");
    ...
    var giveawayEmbed = new discord.RichEmbed()
        .setTitle("🎉 **РОЗЫГРЫШ** 🎉")
        .setFooter(`Конец розыгрыша: ${dateTime}`)
        .setDescription(item);
    ...
    setTimeout(function () {
        ...
        if (peopleReacted.length == 0) {
            return message.channel.send("Никто не выиграл в этом розыгрыше!");
        }

        // Tijdelijk kijken we na als er te wienig mensen hebben mee gedaan aan de wedstrijd.
        if (peopleReacted.length < winnerCount) {
            return message.channel.send("Слишком мало людей, которые участвовали, поэтому бот выиграл.");
        }
        ...
        for (var i = 0; i < winners.length; i++) {
            message.channel.send("Поздравляем, " + winners[i] + `! Вы выиграли **${item}**.`);
        }
